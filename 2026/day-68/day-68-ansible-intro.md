...
1. What is configuration management? Why do we need it?
   - Think of configuration management like version control for your entire system's state, not just its code.
   - Why do we need it
     - Reproducibility 
...
2. How is Ansible different from Chef, Puppet, and Salt?
    ...
    | --------------------- | ---------- | ----------------------- | ------------------- | ----------------- |
    | Agent required        | No         | Yes                     | Yes                 | Yes (usually)     |
    | Communication         | Push (SSH) | Pull                    | Pull                | Push + Pull       |
    | Language              | YAML       | Ruby (DSL)              | Puppet DSL          | YAML + Jinja      |
    | Learning curve        | Low        | High                    | Medium-High         | Medium            |
    | Central server needed | No         | Yes (Chef Infra Server) | Yes (Puppet Server) | Yes (Salt Master) |

3. What does "agentless" mean? How does Ansible connect to managed nodes?
   - "Agentless" means that no persistent software needs to be installed on the machines you want to manage.
   - we can connect the nodes using SSH
...
```
**YES I AM ABLE TO SSH ALL MY WORKER NODES**
...
- Intalled ansible on control node.
...
```
**[inventory.ini](./Task-4/inventory.ini)**
...
**Document:** What does `--become` do? When do you need it?
 - We use become to perform admin actions like root/sudo.
...
db
```
**[inventory.ini](./Task-6/inventory.ini)**
...
```
**[ansible.cfg](./Task-6/ansible.cfg)**
