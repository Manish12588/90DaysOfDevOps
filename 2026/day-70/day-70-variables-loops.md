...
**Verify:** Does the CLI variable override the playbook variable?
 - **Yes CLI variable overrides the playbook variable.**
---
...
## Ansible Variable Precedence (Low → High)
| Priority     | Source                     | Example                               |
| ------------ | -------------------------- | ------------------------------------- |
| 1 ← Lowest   | `role defaults`            | `roles/nginx/defaults/main.yml`       |
| 2            | `group_vars/all.yml`       | applies to all hosts                  |
| 3            | `group_vars/groupname.yml` | applies to specific group             |
| 4            | `host_vars/hostname.yml`   | applies to specific host              |
| 5            | `inventory vars`           | defined in `inventory.ini`            |
| 6            | `playbook vars:`           | defined in playbook                   |
| 7            | `vars_files:`              | external vars file loaded in playbook |
| 8            | `register`                 | output of a task                      |
| 9            | `set_fact`                 | dynamically set in a task             |
| 10 ← Highest | `extra vars -e`            | passed via CLI                        |

```
role defaults             ← easiest to override
     ↓
group_vars/all.yml
     ↓
group_vars/web.yml
...
**Document:** Name five facts you would use in real playbooks and why.
  ...
  | Fact                           | Returns            | Use Case                    |
  | ------------------------------ | ------------------ | --------------------------- |
  | `ansible_distribution`         | `Ubuntu`, `CentOS` | OS based task conditions    |
  | `ansible_memtotal_mb`          | `3984`             | Memory based conditions     |
  | `ansible_default_ipv4.address` | `192.168.1.10`     | Dynamic config files        |
  | `ansible_hostname`             | `web-server-1`     | Labeling and identification |
  | `ansible_processor_vcpus`      | `2`                | Performance tuning          |
---
...
- name: Loops demo
  ...
  tasks:
    ...
    - name: Print each user created
      ...
      loop: "{{ users }}"
```
**NOTE:  wheel is a RHEL/CentOS convention. Ubuntu equivalent is sudo group.**
...
|                       | `with_items`                     | `loop`                          |
| --------------------- | -------------------------------- | ------------------------------- |
| Status                | Old — still works but deprecated | Modern — recommended            |
| Introduced            | Ansible 1.x                      | Ansible 2.5+                    |
| Syntax                | `with_items`                     | `loop`                          |
| Flattens nested lists | ✅ Automatically                  | ❌ Use `flatten` filter manually |
