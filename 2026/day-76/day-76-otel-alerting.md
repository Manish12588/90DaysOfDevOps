...
2. **Create an alert rule in Grafana:**
   ...
   - Query: `container_memory_usage_bytes{name="my-notes-app"} / 1024 / 1024`
...
- **Prometheus Alerts** are defined in YAML rule files loaded by Prometheus itself. They evaluate PromQL expressions on a schedule and transition through OK → PENDING → FIRING states. Alertmanager then handles routing, grouping, and sending notifications.
 
- **Grafana Alerts** are defined inside Grafana dashboards or the Grafana Alerting UI. They can query multiple datasources (Prometheus, Loki, MySQL, etc.) and have their own built-in notification system.

**Side-by-Side Comparison**
 
| Factor                      | Prometheus Alerts              | Grafana Alerts                       |
| --------------------------- | ------------------------------ | ------------------------------------ |
| **Where defined**           | `alert-rules.yml` on disk      | Grafana UI or provisioning YAML      |
| **Query language**          | PromQL only                    | PromQL, LogQL, SQL, and more         |
| **Data sources**            | Prometheus metrics only        | Any Grafana datasource               |
| **Notification routing**    | Alertmanager (complex routing) | Grafana contact points               |
| **Version controlled**      | Naturally (it's a file)        | Requires extra export steps          |
| **Works without Grafana**   | ✅ Yes — fully independent      | ❌ No — needs Grafana running         |
| **Multi-datasource alerts** | ❌ No                           | ✅ Yes                                |
| **Dashboard integration**   | ❌ No visual link               | ✅ Alert annotations appear on panels |
| **Silencing/grouping**      | Alertmanager handles this      | Built into Grafana Alerting          |
 
**When to Use Each**
 
**Use Prometheus Alerts when:**
 
- Alerting purely on infrastructure metrics (CPU, memory, disk, `up == 0`)
- You want alerts to work even if Grafana goes down
- You need complex routing — silence rules, inhibition, grouping by team
- Your rules should live in Git alongside your Terraform/Ansible code
- You're in a team where engineers manage alerts as code

**Use Grafana Alerts when:**
 
- You need to alert across multiple datasources — e.g., "error rate in Loki AND latency spike in Prometheus"
- You want non-engineers to manage alerts through the UI
- You want alert state to be visible directly on dashboard panels as annotations
- You're alerting on log patterns (LogQL) — Prometheus can't do this at all
