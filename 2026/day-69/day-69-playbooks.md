...
**[install-nginx.yml](./Task-1/install-nginx.yml)**
...
**NOTE:** Inside install-nginx.html file update the dest path with '/var/www/html/index.html'

```bash
curl http://13.126.201.29
```
...
- name: Ensure Nginx is running
 ...
 ansible-playbook -i inventory.ini essential-modules.yml --limit -web
```
...
**Document:** What is the difference between `command` and `shell`? When should you use each?
 ...
 - `command`: Runs a command directly on the OS — no shell, no pipes, no redirects.
 - `shell`: Runs a command through /bin/sh — supports pipes |, redirects >, variables $, wildcards *.

    **When to Use**

    | Situation                               | Use         |
    | --------------------------------------- | ----------- |
    | Simple straightforward command          | `command`   |
    | Need pipes `\|` or redirects `>`        | `shell`     |
    | Using `&&` or `\|\|` chaining           | `shell`     |
    | Using `$VARIABLES`                      | `shell`     |
    | Security is a concern                   | `command`   |
    | Native module exists (`apt`, `service`) | **Neither** |
...
**[nginx-config.yml](./Task-4/nginx-config.yml)**

**[nginx.conf](./Task-4/nginx.conf)**
...
**Verify:** Run it twice and compare the output. Does the handler run both times?
 - No in second run handler does not executed.
...
```bash
ansible web -i inventory.ini -m command -a "which mysql"
```
![](Images/Task-6_Step-4.png)

```bash
ansible db -i inventory.ini -m command -a "which mysql"
```
