...
- Loki indexes only metadata labels (e.g., job, container, namespace, level) — not the content of log lines. The actual log content is stored as compressed chunks in object storage (like S3) and is only read when you query it.
...
```
**[loki-config.yml](./Task-2/loki-config.yml)**
...
- Write a LogQL query that finds all error logs from the notes-app container in the last 1 hour
  ```bash
  count_over_time({container_name="my-notes-app"}[60m])
  ```
  ![](Images/Task-5_Step-9.png)

- Then write another query that counts how many error lines per minute.
  ```bash
  count_over_time({container_name="notes-app"} |~ "(?i)error|404|exception"[1m])
  ```
  ![](Images/Task-5_Step-10.png)
---
...
**With Grafana — Unified System**
- Step 1 — Alert fires. You open one Grafana dashboard — metrics and logs are already side by side.
- Step 2 — You see the CPU spike on the top panel and the corresponding error logs on the bottom panel — same timestamp, same screen.
- Step 3 — You click the spike on the graph. Grafana zooms both the metrics panel and the logs panel to that exact time window automatically.
- Step 4 — You read the log line: Exception: database connection pool exhausted. Root cause found.
- Step 5 — You share a single Grafana dashboard URL with your team. Everyone sees the same view instantly.

***Total time to root cause: 3–5 minutes.***
