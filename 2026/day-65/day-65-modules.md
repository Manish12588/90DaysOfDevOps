...
| ---------------------- | -------------------------------------------------- | ----------------------------------------------------------- |
| **What it is**         | The main directory where you run `terraform apply` | A reusable block of Terraform code called by another module |
| **Who calls it**       | Nobody — it's the entry point                      | Called by the root module (or another module)               |
| **Where it lives**     | Your working directory                             | Local path, Git repo, or Terraform Registry                 |
| **Has its own state?** | Yes                                                | No — shares the root module's state                         |
...
[main.tf](./terraform-files/task-2/main.tf)

[variables.tf](./terraform-files/task-2/variables.tf)

[outputs.tf](./terraform-files/task-2/outputs.tf)
...
[variables.tf](./terraform-files/task-3/variables.tf)

[main.tf](./terraform-files/task-3/main.tf)

[outputs.tf](./terraform-files/task-3/outputs.tf)
...
terraform apply
```
[main.tf](./terraform-files/task-4/root/main.tf)         #main.tf file from root directory
...
terraform apply
```
[task-5_main.tf](./terraform-files/task-5/main.tf)
...
terraform state list | Select-String "module.vpc" | Measure-Object -Line
```
...
```
[main.tf](./terraform-files/task-6/main.tf)
