  ...
  handlers:
    - name: Restart Nginx
      service:
        ...
        state: restarted
```
**[nginx-vhist-conf.j2](./Task-1/nginx-vhost.conf.j2)**

**[template-demo.yml](./Task-1/template-demo.yml)**
...
**Document:** What is the difference between `vars/main.yml` and `defaults/main.yml`?
 - Both files define variables for a role, but the differ in priority and intent
 - `defaults/main.yml` Low Priority Defaults
   - Holds default values that are meant to be easily overridden
   - Has the lowest precedence of any variable in Ansible
   - Use this for values you expect users to customize
 
 - `vars/main.yml`  High Priority Variables
   - Holds variables that are internal to the role and not meant to be overridden
   - Has much higher precedence — overrides inventory vars, group vars, host vars, etc.
   - Use this for values that should stay fixed regardless of user input 
...
```bash
ansible-playbook -i <path of inventory.ini> docker-setup.yml
```
...
**Document:** Why use a `requirements.yml` instead of installing roles manually?
 - To ensure everyone in team install the exact same version.
 - All roles and dependency are declared in one file
 - It would help in team collabration, New team member can run just one command to get all dependency.
...
```
**[db-setup.yml](./Task-5/db-setup.yml)**
...
| ----------------- | ---------------------------- | -------------------------- |
| Input method      | Manual typing                | Reads from a file          |
| Human interaction | Required                     | Not required               |
| CI/CD pipelines   | ❌ Breaks automation          | ✅ Fully automated          |
| Use case          | Local/dev runs               | Production & pipelines     |
| Security          | Password in terminal history | File with `chmod 600`      |
| Git commit        | Never commit password        | Never commit password file |

**When to use which?**

- **`--ask-vault-pass`** → Local development, one-off runs, learning/testing
- **`--vault-password-file`** → CI/CD pipelines, Jenkins, GitHub Actions, GitLab CI, any automated deployment
...
```bash
ansible-playbook site.yml
ansible-playbook -i ../ansible-practice/inventory.ini site.yml --ask-vault-password
```
...
**COMMAND TO VERIFY ON WHICH SERVER PLAYBOOK EXECUTED**
```bash
ansible db -i ../ansible-practice/inventory.ini -m command -a "hostname -I" --ask-vault-password
```
