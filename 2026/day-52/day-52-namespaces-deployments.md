...
**Verify:** How many pods are running in `kube-system`?
 - There are total 10 pods are running inside `kube-system`
  
```bash
kubectl get pods -n kube-systen
```
...
[namespace.yml](./yaml/namespace.yml)
...
**Verify:** Does `kubectl get pods` show these pods? What about `kubectl get pods -A`?
        ...
        `kubectl get pods` : says "No resources found in default namespace."

        `kubectl get pods -A` : Fecth all the pods from all namespaces.
...
**Verify:** Is the replacement pod's name the same as the one you deleted, or different?
 - No the newly created pod name is different.
